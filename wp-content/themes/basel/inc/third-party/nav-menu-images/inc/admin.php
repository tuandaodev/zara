...
class Nav_Menu_Images_Admin extends Nav_Menu_Images {
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'nmi-scripts', 'nmi_vars', array(
				'alert' => esc_html__( 'You need to set an image as a featured image to be able to use it as an menu item image', 'basel' )
			)
		);
		add_thickbox();
	...
	public function _wp_post_thumbnail_html( $content, $post_id ) {
		...
		if ( has_post_thumbnail( $post->ID ) )
			...
			$link = esc_html__( 'Upload image', 'basel' );
	...
	function attachment_fields_to_edit( $form_fields, $post ) {
		...
		$form_fields['nmihover'] = array(
			'label'        => esc_html__( 'Used on hover?', 'basel' ),
			'input'        => 'html',
			...
			'value'        => $is_hover,
			'helps'        => esc_html__( 'Should this image be used on hover', 'basel' ),
			'show_in_edit' => false
		);
		...
		$form_fields['nmiactive'] = array(
			'label'        => esc_html__( 'Used when active?', 'basel' ),
			'input'        => 'html',
			...
			'value'        => $is_active,
			'helps'        => esc_html__( 'Should this image be used when menu item is active', 'basel' ),
			'show_in_edit' => false
		);
